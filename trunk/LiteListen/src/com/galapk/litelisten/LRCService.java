		...
		if (IsLyricExist)
		{
			...
			{// 如果本次和上一次处在一个index下，则表示重复处理，所以忽略（歌曲开头0除外）
				...
				if (main.getScreenOrantation() == 1 || main.getScreenOrantation() == 3)
					msg.what = -main.getTxtLRC().getLineHeight() * (index + ClearlyLineNumber - 1 + LineCount) + 120 + main.getTxtLRC().getLineHeight() * ClearlyLineNumber / 2; // 横屏偏移120dip
				else
					msg.what = -main.getTxtLRC().getLineHeight() * (index + ClearlyLineNumber - 1 + LineCount) + 300 + main.getTxtLRC().getLineHeight() * ClearlyLineNumber / 2; // 竖屏偏移300dip
